...
DROP DATABASE IF EXISTS `projet-libre`;
DROP DATABASE IF EXISTS `projet-libre_test`;
CREATE DATABASE `projet-libre` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
...
GRANT ALL ON `projet-libre_test`.* TO 'application'@'%' IDENTIFIED BY 'passw0rd';

GRANT ALL ON `projet-libre`.* TO 'application'@'localhost' IDENTIFIED BY 'passw0rd';
...
-- 	name VARCHAR(100),
-- 	version INTEGER DEFAULT 1,
-- 	CONSTRAINT `FK_Role_User` FOREIGN KEY (idUser) referenceS User (idUser) ON DELETE CASCADE
--  )ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
-- INSERT INTO User(civility,firstname,lastname,username,password,phoneNumber) VALUES ('Mr','Jerome','Cantin','admin@gmail.com','passw0rd','0655554896');
-- INSERT INTO Role(idUser, name) VALUES ((SELECT MAX(idUser) FROM User), "Admin");